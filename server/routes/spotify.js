...
router.post("/playlist", async (req, res) => {
  ...
  try {
    // Search Spotify for each track to get its URI
    const uris = [];
    for (const t of tracks) {
      ...
      try {
        ...
        return res.status(502).json({ error: "Spotify search returned unexpected response" });
      }
    ...
    if (!playlistId) {
      const createRes = await fetch("https://api.spotify.com/v1/me/playlists", {
        ...
        body: JSON.stringify({
          name: name || "PlaylistGenerator – " + new Date().toLocaleDateString(),
